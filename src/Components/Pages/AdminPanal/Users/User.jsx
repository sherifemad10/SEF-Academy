...
// import UserList from "../../../services/AllUser/UserList"
...
import MobilSidebar from "./Mobile/MobilSidebar/MobilSidebar"
import UsercardMobile from "./Mobile/UserCardMobile/UsercardMobile"
...
const User = () => {
  ...
  const handleEdit = (user,id) => {
    navigate(`/updatauser/${id}`, { state: { user } });
  };

    // Filter Users 
    const filteredUsers = value.docs.filter(item => 
      item.data().fristName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      item.data().SecondName.toLowerCase().includes(searchQuery.toLowerCase())
    );
...
<div className="articles-wrapper">
        <div className="articles-contant container">
          <HeaderAdmin linkto={addUser} Title={Title} />
                  ...
                  <tbody>

                    {
                      filteredUsers.slice(0,6).map((item) => (
                        <tr key={item.id}>
                      <td>{item.data().fristName} {item.data().SecondName}</td>
                      {/* <td>{item.state}</td> */}
                      <td>
                        {
                          item.data().state === 'Active' ? <button id="active">{item.data().state}</button> : <button id="Inactive">{item.data().state}</button>
                        }
                      </td>
                      ...
                      <td className="action">
                        ...
                        {/* end edit btn */}
                        <button onClick={() => UsehandleDelete(item.id)} className="button">
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 69 14"
