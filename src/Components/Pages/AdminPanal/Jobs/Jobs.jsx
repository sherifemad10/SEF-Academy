...
import JobsMobileCard from './Mobile/JobsMobileCard'

const Jobs = () => {
    const createArticle = "/createJob"
  ...
  const [value, loading, error] = useCollection(jobsQuery);
  ...
  if (error) {
    ...
    const filteredArticles = value.docs.filter(item => {
      ...
      return company.includes(searchQuery.toLowerCase()) || position.includes(searchQuery.toLowerCase());
    });

    // edit
    const handleEdit = (job,id) => {
      navigate(`/updataJob/${id}`, { state: { job } });
    };
        ...
        <div className="articles-contant container">
          ...
          <div className="article-body">
            ...
            <div className="posted-artical">
              ...
              {filteredArticles.slice(0, 7).map((user) => (
                  <JobsMobileCard
                    key={user.id}
                    userid={user.data().id}
                ...
                <table className='jobs-table'>
                  ...
                  <tbody>

                    {
                      ...
                      <td>
                        {
                          item.data().state === 'Open' ? <button className="open">{item.data().state}</button> : <button className="close">{item.data().state}</button>
                        }
                      </td>
