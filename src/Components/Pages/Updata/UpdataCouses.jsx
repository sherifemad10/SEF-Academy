...
import "../Add/AddCourses/AddCourses.css";
...
const UpdataCourses = () => {
  ...
  const articleToEdit = location.state || "null";
  ...
  useEffect(() => {
    if (articleToEdit) {
      ...
      setLevel(articleToEdit.level);
      setLessons(articleToEdit.lesson);
      setLanguage(articleToEdit.language);
      setStartData(articleToEdit.startData);
      setDuration(articleToEdit.duration);
      setCertificate(articleToEdit.certificate);
      setIntroduction(articleToEdit.introduction);
      setAssessment(articleToEdit.assessment);
      setRequirements(articleToEdit.requirements);
      setMaterials(articleToEdit.materials);
      setImg(articleToEdit.img);
      setLessonsArray(articleToEdit.lessonscontant);
    }
  }, [articleToEdit]);
  ...
  const editt = () => {
    if (articleToEdit) {
      console.log("updata");
      const dataedit = doc(db, "Courses", id);
      updateDoc(dataedit, {
        name: name,
        level: level,
        lesson: lesson,
      ...
      navigate("/coursesAdded");
      toast("Courses Updata successfully", {
        position: "top-right",
        autoClose: 5000,
            ...
            <div className="add-courses-form">
              <div className="add-courses-header">
                <div className="coursess">
                  <h5>Updata Courses Details</h5>
                </div>

                <div className="publish">
                  <button onClick={editt} className="publish-btn">
                    Updata
                  </button>
                </div>
              </div>{" "}
